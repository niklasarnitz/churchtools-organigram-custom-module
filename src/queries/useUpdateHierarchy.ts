...
export const useUpdateHierarchy = () => {
	...
	return useMutation({
		mutationFn: async ({ childrenIds, groupId, parentId }: UpdateHierarchyPayload) => {
			...
			if (parentId) {
				promises.push(churchtoolsClient.put(`/groups/${String(groupId)}/parents/${String(parentId)}`, {}));
			}

			for (const childId of childrenIds) {
				promises.push(churchtoolsClient.put(`/groups/${String(childId)}/parents/${String(groupId)}`, {}));
			}
