            ...
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    ...
                    <DialogDescription className="mt-1.5">
                        <span className="inline-block rounded-full bg-slate-100 dark:bg-slate-800 px-2.5 py-0.5 text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">
                            Schritt {currentIndex + 1} von {orphanedGroups.length}
                        </span>
                        <br />
                        Ordne die Gruppe <strong className="text-slate-900 dark:text-slate-100">{currentGroup?.name}</strong> zu.
                    </DialogDescription>
                </DialogHeader>
                ...
                {currentGroup && (
                    <div className="grid gap-6 py-4">
                        <div className="grid gap-2">
                            <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Übergeordnete Gruppe (Eltern)</label>
                            <Combobox
                        ...
                        <div className="grid gap-2">
                            <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Untergeordnete Gruppen (Kinder)</label>
                            <MultiSelect
                ...
                <DialogFooter className="flex flex-row items-center justify-between border-t border-slate-100 dark:border-slate-800 pt-4 sm:justify-between">
                    <div className="flex gap-2">
                        <Button
                            disabled={currentIndex === 0}
                            onClick={() => { resetSelection(currentIndex - 1); }}
                            size="sm"
                            variant="outline"
                        >
