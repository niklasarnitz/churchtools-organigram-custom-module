...
export const FloatingHeader = React.memo(() => {
    ...
    const sortedNodes = useMemo(() => {
        return (nodes as Node<PreviewGraphNodeData>[]).slice().sort((a, b) => 
            a.data.title.localeCompare(b.data.title)
        );
    }, [nodes]);

    return (
        <div className="absolute left-1/2 top-6 z-50 flex w-full max-w-2xl -translate-x-1/2 items-center gap-4 px-4">
