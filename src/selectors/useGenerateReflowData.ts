				...
				if (layoutAlgorithm === LayoutAlgorithm.elkRadial) {
					...
					for (const [parentId, children] of childrenMap) {
						...
						for (const child of children) {
							if (!visibleNodeIds.has(child.id)) continue;
					...
					if ((window as any).__DEBUG_RADIAL) {
						console.log(`=== ELK_RADIAL Layout ===`);
						console.log(`Visible nodes: ${reflowNodes.length}`);
						console.log(`Relations built from tree: ${elkRelations.length}`);
					}

					result = await layoutElk(reflowNodes, elkRelations, layoutAlgorithm, nodeSizes);
				} else if (layoutAlgorithm === LayoutAlgorithm.FLAT_RADIAL) {
					...
					if (reflowNodes.length === 0) {
						...
						if ((window as any).__DEBUG_RADIAL) {
							...
							console.log(`Total entries in hierarchiesByGroupId: ${Object.keys(hierarchiesByGroupId).length}`);
							let totalChildrenInHierarchies = 0;
							for (const [parentIdStr, hierarchy] of Object.entries(hierarchiesByGroupId)) {
								const childCount = hierarchy?.children?.length || 0;
								totalChildrenInHierarchies += childCount;
								const childrenStr = childCount > 0 ? '→ ' + (hierarchy?.children || []).join(', ') : '';
								console.log(`  ${parentIdStr}: ${childCount} children ${childrenStr}`);
							}
							console.log(`Total parent-child relations in hierarchies: ${totalChildrenInHierarchies}`);
							
							console.log(`\n=== CHILDREN MAP FROM FULL HIERARCHIES (including hidden nodes) ===`);
							for (const [parentId, children] of childrenMap) {
								const parentNode = nodeDataById.get(parentId);
								console.log(`Parent ${parentId} (${parentNode?.title || '[hidden]'}): ${children.length} children`);
								for (const child of children) {
						...
						if ((window as any).__DEBUG_RADIAL) {
							...
							const calculateMaxDepth = (nodeId: number, visited = new Set<number>()): number => {
								...
								return 1 + Math.max(...children.map(c => calculateMaxDepth(c.id, new Set(visited))));
							};
							maxDepth = calculateMaxDepth(rootNode.id);
							
							for (const [parentId, children] of childrenMap) {
								const parentNode = nodeDataById.get(parentId);
								console.log(`Parent ${parentId} (${parentNode?.title}): ${children.length} children`);
							...
							const childrenWithOwnChildren = Array.from(childrenMap.values()).filter(c => c.length > 0).length;
							console.log(`Direct children of root: ${directChildrenOfRoot}, Parents with their own children: ${childrenWithOwnChildren}`);
							
							if (childrenWithOwnChildren === 0 && directChildrenOfRoot > 0) {
								console.warn(`⚠️ STRUCTURE IS FLAT: Root has ${directChildrenOfRoot} children but none of them have children!`);
								console.warn(`This likely means intermediate nodes are filtered out or relations are missing.`);
							}
						}
						...
						if ((window as any).__DEBUG_RADIAL) {
							...
							(window as any).__RAY_STRUCTURE = exportRayStructure(nodePolarMap, childrenMap, nodeDataById);
						}
					...
					result = await layoutElk(reflowNodes, reflowEdges, layoutAlgorithm, nodeSizes);
				}
				}
