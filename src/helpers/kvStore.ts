...
export async function getUserSettings<T>(categoryShorty: string, categoryName: string): Promise<T | undefined> {
...
export async function saveUserSettings(categoryShorty: string, categoryName: string, settings: object): Promise<void> {
	const category = await getOrCreateCustomDataCategory(categoryShorty, categoryName);
	const mId = category.moduleId;
